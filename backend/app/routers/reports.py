...
async def generate_audit_package(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        for ev in evidence:
            ...
            evidence_data.append({
                ...
                "frameworks": ev.frameworks, "control_ids": ev.control_ids,
                "last_reviewed": str(ev.last_reviewed), "owner": ev.owner,
                "summary": ev.summary, "snippets": ev.snippets,
        ...
        policies_data = [
            {"id": p.id, "title": p.title, "category": p.category,
             "content": p.content, "status": p.status, "version": p.version,
        ...
        # Pentests JSON
        pentests_data = [
            {"id": pt.id, "title": pt.title, "scope": pt.scope,
             "methodology": pt.methodology, "status": pt.status,
        ...
        # Answers JSON
        answers_data = [
            {"id": a.id, "question": a.question, "answer_text": a.answer_text,
             "confidence": a.confidence, "needs_human_review": a.needs_human_review,
             "confidence_rationale": a.confidence_rationale,
             "citations": a.citations or [], "freshness": a.freshness or []}
        ...
        for ev in evidence:
            ...
            soc2_lines.append(f"- Type: {ev.type}  |  Owner: {ev.owner}  |  Last Reviewed: {ev.last_reviewed} ({days_ago}d — {status})")
        ...
        for pt in pentests:
            ...
            for f in (pt.findings or []):
                soc2_lines.append(f"  - [{f.get('severity', '?').upper()}] {f.get('title', 'Untitled')} ({f.get('status', '?')})")
            soc2_lines.append("")
    ...
    audit = AuditLog(
        ...
        action="export",
        changes={"evidence_count": len(evidence), "answers_count": len(all_answers), "policies_count": len(policies), "pentests_count": len(pentests)},
    )
    db.add(audit)
    await db.commit()
